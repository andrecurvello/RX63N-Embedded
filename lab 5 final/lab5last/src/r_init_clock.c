...
void R_INIT_Clock(void)
{
    ...
    if (0 == SYSTEM.RSTSR1.BIT.CWSF)        /* Cold start */
    {
        CGC_subclk_as_RTC();                /* the sub-clock pattern D */
    }
    else                                    /* Warm start */
    {
        no_use_subclk_as_sysclk();          /* Processing when not using the sub-clock as the system clock */
    }

    /* ---- Wait for 2 sub-clock cycles (approx. 61 us) ---- */
    cmt0_wait((SUB_CLOCK_CYCLE*2/FOR_CMT0_TIME)+1);
    ...
    b7:b4   UCK      - USB Clock (UCLK) Select     - divide-by-2
